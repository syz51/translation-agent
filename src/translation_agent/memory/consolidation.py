...
class DeterministicMemoryConsolidationBackend:
    ...
    def consolidate_batch(self, batch: MemoryWriteBatch) -> MemoryConsolidation:
        ...
        return MemoryConsolidation(
            ...
            source_tenant_id=_metadata_string(batch.metadata, "tenant_id"),
            source_project_id=_metadata_string(batch.metadata, "project_id"),
        ...
        *,
        ...
        for index, write in enumerate(writes, start=1):
            ...
            entry = MemoryEntry(
                ...
                kind=write.kind,
                memory_subtype=write.memory_subtype,
                ...
                promotion_status=write.promotion_status,
                evidence_count=write.evidence_count,
                supporting_run_count=write.supporting_run_count,
                supporting_asset_count=write.supporting_asset_count,
                supporting_project_count=write.supporting_project_count,
                contradiction_count=write.contradiction_count,
                series_id=write.series_id,
                franchise_id=write.franchise_id,
                speaker_ids=write.speaker_ids,
                content_type=write.content_type,
                topic_tags=write.topic_tags,
                style_profile_id=write.style_profile_id,
                entity_keys=write.entity_keys,
                term_keys=write.term_keys,
                applicability_scope=write.applicability_scope,
                evidence_refs=write.evidence_refs,
                last_validated_at=write.last_validated_at,
                success_count=write.success_count,
                failure_count=write.failure_count,
                validation_status=write.validation_status,
                promotion_ref=write.promotion_ref,
                quality_gate_status=write.quality_gate_status,
                typed_metadata=write.typed_metadata,
                metadata={
                    **batch.metadata,
                    **write.metadata,
                    "batch_id": batch.batch_id,
